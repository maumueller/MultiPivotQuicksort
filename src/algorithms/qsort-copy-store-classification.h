 ...
 * Store classifications and non-inplace partitioning.
 *
 ******************************************************************************
...
struct ClassifySimple
{
    ...
    inline void
    classify(Iterator begin, Iterator end, uint8_t* bktout) const
    {
        for (Iterator iter = begin; iter != end; )
        {
            value_type key = *iter++;
            uint8_t b = find_bkt_tree(key);
    ...
    inline void
    find_bkt_tree_unroll(const value_type key[U], uint8_t obkt[U]) const
    {
    ...
    __attribute__((optimize("unroll-all-loops")))
    inline void
    classify_unroll(Iterator begin, Iterator end, uint8_t* bktout) const
    {
...
void sample_sort(Iterator begin, Iterator end)
{
    ...
    uint8_t* bktcache = new uint8_t[n];
    ...
    value_type* cparr = new value_type[n];

    for (size_t i = 0; i < n; i++)
    {
	cparr[bktnext[bktcache[i]]++] = *(begin + i);
	g_assignments++;
    }

    memcpy(begin, cparr, n * sizeof(value_type));
    // g_assignments += n - 127;
...
CONTESTANT_REGISTER_ALL(issort, "qsort-copy-store", "Super Scalar Sample Sort storing classifications")
